//задание 1 и 2

/*1.Создайте переменные: name, age, city, phone, email, company; и получите через ф-цию
prompt() соответствующие значения: ваше имя, возраст, город проживания и т.д.
Выведите на экран фразу: «Меня зовут %Имя%. Мне %Возраст% лет. Я проживаю в
городе %Город% и работаю в компании %Компания%. Мои контактные данные:
%Телефон%, %Почта%.».
2.Определите по введенному возрасту (исп. значение из задания 1) год рождения.
Выведите на экран «%Имя% родился в %Год% году.».
*/

/*let name = prompt('Как Вас зовут?'),
    age = prompt('Укажите свой возраст.'),
    city = prompt('Укажите город вашего проживания'),
    phone = prompt('Укажите ваш телефон.'),
    email = prompt('Укажите ваш email.'),
    company = prompt('Укажите компанию, в которой вы работаете.');



document.write(`Меня зовут ${name}. Мне ${age}. Я проживаю в городе ${city} и работаю в  компании ${company}. Мои контактные данные: ${phone}, ${email}`);


let year = 2023;
if (age == "" ||  0 ) {
    console.log('Пожалуйста введите корректный возраст');
} else { console.log(year - age);
}

//задание 3
/*Дана строка из 6-ти цифр. Проверьте, что сумма первых трех цифр равняется сумме
вторых трех цифр. Если это так - выведите 'да', в противном случае выведите 'нет'. */

let number = '297532';
let suma = +number[0] + +number[1] + +number[2];
let sumb = +number[3] + +number[4] + +number[5];

if (suma == sumb){
    console.log('yes');
} else {
    console.log('no');
}

console.log()

// задание 4
/*Если переменная a больше нуля, то выведите 'Верно', иначе выведите 'Неверно'.
Проверьте работу скрипта при a, равном 1, 0, -3.*/


let a = prompt('Введите число');
if (a > 0) {
    console.log('Верно');
} else {
    console.log('Неверно');
}

//задание 5 , заменены переменные
/*Создайте переменные a=10 и b=2. Выведите их сумму, разность, произведение,
частное, а также, если сумма этих чисел больше 1, то возведите полученную сумму в
квадрат.*/


let numa = 10,
    numb = 2;
console.log(numa+numb);
console.log(numa-numb);
console.log(numa*numb);
console.log(numa/numb);

if ((numa + numb) > 1) {
    console.log((numa+numb)**2);
} else {
    console.log(numa+numb);
}

// задание 6 заменены переменные
/*Если переменная a (из задания 5) больше 2 и меньше 11, или переменная b (из задания
5) больше или равна 6 и меньше 14, то выведите 'Верно', в противном случае выведите
'Неверно'.*/

let c = prompt('Введите число c'),
    d = prompt('Введите число d');

if (2 < c &&  c < 11 || 6 <= d && d < 14) {
    console.log('Верно');
} else {
    console.log('Неверно');
}

// задание 7
/*В переменной n лежит число от 0 до 59. Определите в какую четверть часа попадает
это число (в первую, вторую, третью или четвертую).*/

let n = prompt('Введите число n от 0 до 59');

if (0 <= n && n <= 15) {
    console.log('I четверть');
} else if(15< n && n <= 30) {
    console.log('II четверть');
} else if(30< n && n <= 45) {
    console.log('III четверть');
} else if(45< n && n <= 59){
    console.log('IV четверть');
} else {
    console.log('Введите корректное значение');
}

//задание 8
/*В переменной day лежит число из интервала от 1 до 31. Определите в какую декаду
месяца попадает это число (в первую, вторую или третью).*/


let day = prompt('Для определения декады месяца введите число от 1 до 31');

if (1 <= day && day <= 10) {
    console.log('I декада');
} else if(11 < day && day <= 20) {
    console.log('II декада');
} else if(21 < day && day <= 31) {
    console.log('III декада');
} else {
    console.log('Введите корректное значение');
}